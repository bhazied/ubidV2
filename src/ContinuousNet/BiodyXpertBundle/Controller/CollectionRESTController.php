 ...
 * @see      CollectionRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Collection")
 */
class CollectionRESTController extends BaseRESTController
{
    /**
     ...
     *
     */
    public function getAction(Collection $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new Collection());
            ...
            $qb->from('BiodyXpertBundle:Collection', 'c_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'c_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'c_.modifierUser = modifier_user.id');
            $textFields = array('collection.name', 'collection.slug');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('collection.', 'c_.', $field);
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('collection.', 'c_.', $field);
    ...
    public function postAction(Request $request)
    {
        ...
        $form = $this->createForm(new CollectionType(), $entity, array('method' => $request->getMethod()));
     ...
     * @return Response
     */
    public function putAction(Request $request, Collection $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Collection entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, Collection $entity)
    {
     ...
     * @return Response
     */
    public function deleteAction(Request $request, Collection $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
            $em->remove($entity);
