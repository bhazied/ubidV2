...
class UserRESTController extends BaseRESTController
{
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('BiodyXpertBundle:User', 'u_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\Country', 'country', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.country = country.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\Licence', 'licence', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.licence = licence.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\DeviceSerialNumber', 'device_serial_number', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.deviceSerialNumber = device_serial_number.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\Language', 'language', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.language = language.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.modifierUser = modifier_user.id');
            $textFields = array('user.username', 'user.phone', 'user.email', 'user.usernameCanonical', 'user.emailCanonical', 'user.firstName', 'user.lastName', 'user.picture', 'user.address', 'user.zipCode', 'user.companyName', 'user.job', 'user.cityName', 'user.serialNumber', 'user.phoneValidationCode', 'user.emailValidationCode', 'user.roles', 'user.confirmationToken');
