...
class TranslationPhysicalActivityRESTController extends BaseRESTController
{
     ...
     *
     */
    public function getAction(TranslationPhysicalActivity $entity)
    {
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('BiodyXpertBundle:TranslationPhysicalActivity', 'tpa_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\PhysicalActivity', 'physical_activity', \Doctrine\ORM\Query\Expr\Join::WITH, 'tpa_.physicalActivity = physical_activity.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'tpa_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'tpa_.modifierUser = modifier_user.id');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('translationPhysicalActivity.', 'tpa_.', $field);
            ...
            $qbList = clone $qb;
            $qb->select('count(tpa_.id)');
            $data['inlineCount'] = $qb->getQuery()->getSingleScalarResult();
            foreach ($order_by as $field => $direction) {
                $field = str_replace('translationPhysicalActivity.', 'tpa_.', $field);
                $qbList->addOrderBy($field, $direction);
            }
            $qbList->select('tpa_');
            $qbList->setMaxResults($limit);
            $qbList->setFirstResult($offset);
            $qbList->groupBy('tpa_.id');
     ...
     * @return Response
     */
    public function putAction(Request $request, TranslationPhysicalActivity $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
     ...
     * @return Response
     */
    public function patchAction(Request $request, TranslationPhysicalActivity $entity)
    {
     ...
     * @return Response
     */
    public function deleteAction(Request $request, TranslationPhysicalActivity $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
            $em->remove($entity);
