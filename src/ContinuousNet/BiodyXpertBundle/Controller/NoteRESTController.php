 ...
 * @see      NoteRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Note")
 */
class NoteRESTController extends BaseRESTController
{
    /**
     * Get a Note entity
     *
    ...
    {
        try {
            $this->createSubDirectory(new Note());
            ...
            $qb->from('BiodyXpertBundle:Note', 'n_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\Patient', 'patient', \Doctrine\ORM\Query\Expr\Join::WITH, 'n_.patient = patient.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'n_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'n_.modifierUser = modifier_user.id');
            $textFields = array('note.comment');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('note.', 'n_.', $field);
            ...
            if (!empty($roles)) {
                foreach ($roles as $role) {
                   if (substr_count($role, 'ACC') > 0) {
                       $qb->andWhere('n_.creatorUser = :creatorUser')->setParameter('creatorUser', $this->getUser()->getId());
                   }
                }
            }
            $qbList = clone $qb;
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('note.', 'n_.', $field);
    ...
    public function postAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new Note();
        $form = $this->createForm(new NoteType(), $entity, array('method' => $request->getMethod()));
