...
class TemplateRESTController extends BaseRESTController
{
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('BiodyXpertBundle:Template', 't_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\LicenceType', 'licence_type', \Doctrine\ORM\Query\Expr\Join::WITH, 't_.licenceType = licence_type.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 't_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 't_.modifierUser = modifier_user.id');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('template.', 't_.', $field);
            ...
            if (!empty($roles)) {
                foreach ($roles as $role) {
                   if (substr_count($role, 'MAN') > 0) {
                       $qb->andWhere('t_.creatorUser = :creatorUser OR t_.shareLevel = :shareLevel')->setParameter('creatorUser', $this->getUser()->getId())->setParameter('shareLevel', 'Everyone');
                   }
                }
            }
            $qbList = clone $qb;
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('template.', 't_.', $field);
    ...
    public function postAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            $authorizedChangeLicenceTypeId = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeLicenceTypeId = true;
                    }
                }
            }
            if (!$authorizedChangeLicenceTypeId) {
                $entity->setLicenceType(null);
            }
            $em->persist($entity);
            $em->flush();
            return $entity;
     ...
     * @return Response
     */
    public function putAction(Request $request, Template $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
     ...
     * @return Response
     */
    public function patchAction(Request $request, Template $entity)
    {
     ...
     * @return Response
     */
    public function deleteAction(Request $request, Template $entity)
    {
        try {
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
