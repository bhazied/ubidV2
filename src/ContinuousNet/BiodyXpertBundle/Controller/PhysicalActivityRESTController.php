...
class PhysicalActivityRESTController extends BaseRESTController
{
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('BiodyXpertBundle:PhysicalActivity', 'pa_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'pa_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'pa_.modifierUser = modifier_user.id');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('physicalActivity.', 'pa_.', $field);
            ...
            $qbList = clone $qb;
            $qb->select('count(pa_.id)');
            $data['inlineCount'] = $qb->getQuery()->getSingleScalarResult();
            foreach ($order_by as $field => $direction) {
                $field = str_replace('physicalActivity.', 'pa_.', $field);
                $qbList->addOrderBy($field, $direction);
            }
            $qbList->select('pa_');
            $qbList->setMaxResults($limit);
            $qbList->setFirstResult($offset);
            $qbList->groupBy('pa_.id');
