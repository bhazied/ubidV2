 ...
 * @see      WordRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Word")
 */
class WordRESTController extends BaseRESTController
{
    /**
     * Get a Word entity
     *
     ...
     *
     */
    public function getAction(Word $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new Word());
            ...
            $qb->from('BiodyXpertBundle:Word', 'w_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'w_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'w_.modifierUser = modifier_user.id');
            $textFields = array('word.name', 'word.key');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('word.', 'w_.', $field);
            ...
            $qbList = clone $qb;
            $qb->select('count(w_.id)');
            $data['inlineCount'] = $qb->getQuery()->getSingleScalarResult();
            foreach ($order_by as $field => $direction) {
                $field = str_replace('word.', 'w_.', $field);
                $qbList->addOrderBy($field, $direction);
            }
            $qbList->select('w_');
            $qbList->setMaxResults($limit);
            $qbList->setFirstResult($offset);
            $qbList->groupBy('w_.id');
    ...
    public function postAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new Word();
        $form = $this->createForm(new WordType(), $entity, array('method' => $request->getMethod()));
     ...
     * @return Response
     */
    public function putAction(Request $request, Word $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Word entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, Word $entity)
    {
