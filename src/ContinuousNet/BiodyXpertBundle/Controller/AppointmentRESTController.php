 ...
 * @see      AppointmentRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Appointment")
 */
class AppointmentRESTController extends BaseRESTController
{
    /**
     * Get a Appointment entity
     *
     ...
     *
     */
    public function getAction(Appointment $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new Appointment());
            ...
            $qb->from('BiodyXpertBundle:Appointment', 'a_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\Patient', 'patient', \Doctrine\ORM\Query\Expr\Join::WITH, 'a_.patient = patient.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'a_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'a_.modifierUser = modifier_user.id');
            $textFields = array('appointment.subject', 'appointment.comment');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('appointment.', 'a_.', $field);
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('appointment.', 'a_.', $field);
    ...
    public function postAction(Request $request)
    {
        ...
        $form = $this->createForm(new AppointmentType(), $entity, array('method' => $request->getMethod()));
     ...
     * @return Response
     */
    public function putAction(Request $request, Appointment $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Appointment entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, Appointment $entity)
    {
        return $this->putAction($request, $entity);
    }

    /**
     * Delete a Appointment entity.
     *
     * @View(statusCode=204)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function deleteAction(Request $request, Appointment $entity)
    {
        try {
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
