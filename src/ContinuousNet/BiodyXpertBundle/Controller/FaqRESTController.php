...
class FaqRESTController extends BaseRESTController
{
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('BiodyXpertBundle:Faq', 'f_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\FaqCategory', 'faq_category', \Doctrine\ORM\Query\Expr\Join::WITH, 'f_.faqCategory = faq_category.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'f_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'f_.modifierUser = modifier_user.id');
            $textFields = array('faq.title', 'faq.slug', 'faq.content');
