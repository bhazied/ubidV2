 ...
 * @see      VariableRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Variable")
 */
class VariableRESTController extends BaseRESTController
{
    /**
     * Get a Variable entity
     *
     ...
     *
     */
    public function getAction(Variable $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new Variable());
            ...
            $qb->from('BiodyXpertBundle:Variable', 'v_');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'v_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BiodyXpertBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'v_.modifierUser = modifier_user.id');
            $textFields = array('variable.name', 'variable.unit', 'variable.color');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('variable.', 'v_.', $field);
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('variable.', 'v_.', $field);
    ...
    public function postAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new Variable();
        $form = $this->createForm(new VariableType(), $entity, array('method' => $request->getMethod()));
     ...
     * @return Response
     */
    public function putAction(Request $request, Variable $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Variable entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, Variable $entity)
    {
     ...
     * @return Response
     */
    public function deleteAction(Request $request, Variable $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
            $em->remove($entity);
