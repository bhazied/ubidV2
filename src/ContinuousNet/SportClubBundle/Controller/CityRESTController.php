 ...
 * @see      CityRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("City")
 */
class CityRESTController extends BaseRESTController
{
    /**
     * Get a City entity
     *
     ...
     *
     */
    public function getAction(City $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new City());
            ...
            $qb->from('SportClubBundle:City', 'c_');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\Country', 'country', \Doctrine\ORM\Query\Expr\Join::WITH, 'c_.country = country.id');
            ...
            $textFields = array('city.name', 'city.nameAr', 'city.nameFr');
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('city.', 'c_.', $field);
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('city.', 'c_.', $field);
    ...
    public function postAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new City();
        $form = $this->createForm(new CityType(), $entity, array('method' => $request->getMethod()));
    ...
    /**
     * Partial Update to a City entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function deleteAction(Request $request, City $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
            $em->remove($entity);
