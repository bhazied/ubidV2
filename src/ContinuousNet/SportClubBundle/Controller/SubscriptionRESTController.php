 ...
 * @see      SubscriptionRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Subscription")
 */
class SubscriptionRESTController extends BaseRESTController
{
    /**
     ...
     *
     */
    public function getAction(Subscription $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new Subscription());
            ...
            $qb->from('SportClubBundle:Subscription', 's_');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\Visit', 'visit', \Doctrine\ORM\Query\Expr\Join::WITH, 's_.visit = visit.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\Package', 'package', \Doctrine\ORM\Query\Expr\Join::WITH, 's_.package = package.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\Price', 'price', \Doctrine\ORM\Query\Expr\Join::WITH, 's_.price = price.id');
            ...
            $textFields = array('subscription.reference', 'subscription.token', 'subscription.msisdn', 'subscription.voucher', 'subscription.serialNumber', 'subscription.currency', 'subscription.userAgent');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('subscription.', 's_.', $field);
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('subscription.', 's_.', $field);
    ...
    public function postAction(Request $request)
    {
        ...
        $form = $this->createForm(new SubscriptionType(), $entity, array('method' => $request->getMethod()));
     ...
     * @return Response
     */
    public function putAction(Request $request, Subscription $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Subscription entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, Subscription $entity)
    {
