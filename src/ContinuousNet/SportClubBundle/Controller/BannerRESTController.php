...
class BannerRESTController extends BaseRESTController
{
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('SportClubBundle:Banner', 'b_');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\BannerType', 'banner_type', \Doctrine\ORM\Query\Expr\Join::WITH, 'b_.bannerType = banner_type.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'b_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'b_.modifierUser = modifier_user.id');
