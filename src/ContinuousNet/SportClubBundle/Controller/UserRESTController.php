...
class UserRESTController extends BaseRESTController
{
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('SportClubBundle:User', 'u_');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\Company', 'company', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.company = company.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\Country', 'country', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.country = country.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\City', 'city', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.city = city.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\Language', 'language', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.language = language.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.modifierUser = modifier_user.id');
            $textFields = array('user.username', 'user.phone', 'user.email', 'user.usernameCanonical', 'user.emailCanonical', 'user.firstName', 'user.lastName', 'user.picture', 'user.address', 'user.zipCode', 'user.job', 'user.facebookKey', 'user.profile', 'user.phoneValidationCode', 'user.emailValidationCode', 'user.roles', 'user.confirmationToken');
    ...
    public function postAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            $authorizedChangeCredentialsExpired = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'SYSTEM') > 0) {
                        $authorizedChangeCredentialsExpired = true;
                    }
                }
            }
            if (!$authorizedChangeCredentialsExpired) {
