 ...
 * @see      QuizRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Quiz")
 */
class QuizRESTController extends BaseRESTController
{
    /**
     * Get a Quiz entity
     *
     ...
     *
     */
    public function getAction(Quiz $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new Quiz());
            ...
            $qb->from('SportClubBundle:Quiz', 'q_');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\QuizType', 'quiz_type', \Doctrine\ORM\Query\Expr\Join::WITH, 'q_.quizType = quiz_type.id');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('quiz.', 'q_.', $field);
            ...
            $qbList = clone $qb;
            $qb->select('count(q_.id)');
            $data['inlineCount'] = $qb->getQuery()->getSingleScalarResult();
            foreach ($order_by as $field => $direction) {
                $field = str_replace('quiz.', 'q_.', $field);
                $qbList->addOrderBy($field, $direction);
            }
            $qbList->select('q_');
            $qbList->setMaxResults($limit);
            $qbList->setFirstResult($offset);
            $qbList->groupBy('q_.id');
    ...
    public function postAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new Quiz();
        $form = $this->createForm(new QuizType(), $entity, array('method' => $request->getMethod()));
