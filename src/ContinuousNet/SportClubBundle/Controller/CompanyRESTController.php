 ...
 * @see      CompanyRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Company")
 */
class CompanyRESTController extends BaseRESTController
{
    /**
    ...
    {
        try {
            $this->createSubDirectory(new Company());
            ...
            $qb->from('SportClubBundle:Company', 'c_');
            ...
            $textFields = array('company.name', 'company.picture', 'company.website', 'company.email', 'company.phone', 'company.fax');
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('company.', 'c_.', $field);
    ...
    public function postAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new Company();
        $form = $this->createForm(new CompanyType(), $entity, array('method' => $request->getMethod()));
