 ...
 * @see      StatRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Stat")
 */
class StatRESTController extends BaseRESTController
{
    /**
     * Get a Stat entity
     *
     ...
     *
     */
    public function getAction(Stat $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new Stat());
            ...
            $qb->from('SportClubBundle:Stat', 's_');
            $qb->leftJoin('ContinuousNet\SportClubBundle\Entity\Table', 'table', \Doctrine\ORM\Query\Expr\Join::WITH, 's_.table = table.id');
            ...
            foreach ($filters as $field => $value) {
                ...
                $_field = str_replace('stat.', 's_.', $field);
            ...
            foreach ($order_by as $field => $direction) {
                $field = str_replace('stat.', 's_.', $field);
    ...
    public function postAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new Stat();
        $form = $this->createForm(new StatType(), $entity, array('method' => $request->getMethod()));
     ...
     * @return Response
     */
    public function putAction(Request $request, Stat $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Stat entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, Stat $entity)
    {
        return $this->putAction($request, $entity);
    }

    /**
     * Delete a Stat entity.
     *
     * @View(statusCode=204)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function deleteAction(Request $request, Stat $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
            $em->remove($entity);
