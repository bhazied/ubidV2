...
app.controller('CompaniesCtrl', ['$scope', '$rootScope', '$stateParams', '$location', '$sce', '$timeout', '$filter', 'ngTableParams', '$state', '$q', '$interpolate', '$localStorage', 'toaster', 'SweetAlert', '$usersDataFactory', '$companiesDataFactory',
function($scope, $rootScope, $stateParams, $location, $sce, $timeout, $filter, ngTableParams, $state, $q, $interpolate, $localStorage, toaster, SweetAlert, $usersDataFactory, $companiesDataFactory) {
    ...
    $scope.getParamValue = function(param, defaultValue) {
        ...
        } else if (angular.isDefined($localStorage.companiesParams[param]) && $localStorage.companiesParams[param] != null) {
    ...
    $scope.isFiltersVisible = $scope.getParamValue('companiesIsFiltersVisible', false);
    $scope.$watch('isFiltersVisible', function() {
        $scope.setParamValue('companiesIsFiltersVisible', $scope.isFiltersVisible);
    });

    $scope.page = 1; // show first page
    $scope.page = $scope.getParamValue('companiesPage', $scope.page);
    $scope.count = 50; // count per page
    $scope.count = $scope.getParamValue('companiesCount', $scope.count);
    $scope.sorting = {'company.name': 'asc'};
    $scope.sorting = $scope.getParamValue('companiesSorting', $scope.sorting);
    $scope.filter = {
    };
    $scope.filter = $scope.getParamValue('companiesFilter', $scope.filter);
    $scope.setParamValue('companiesPage', $scope.page);
    $scope.setParamValue('companiesCount', $scope.count);
    $scope.setParamValue('companiesSorting', $scope.sorting);
    $scope.setParamValue('companiesFilter', $scope.filter);
    $scope.tableParams = {
        page: $scope.page,
        count: $scope.count,
    ...
    $scope.tableParams = new ngTableParams($scope.tableParams, {
        getData: function ($defer, params) {
            ...
            $scope.setParamValue('companiesIsFiltersVisible', $scope.isFiltersVisible);
            $scope.setParamValue('companiesPage', current);
            $scope.setParamValue('companiesCount', limit);
            $scope.setParamValue('companiesSorting', order_by);
            $scope.setParamValue('companiesFilter', filters);
            var http_params = {
                offset: offset,
                limit: limit
            };
    ...
    $scope.delete = function(row) {
        SweetAlert.swal({
            ...
            if (isConfirm) {
                $companiesDataFactory.remove({id: row.id}).$promise.then(function(data) {
                    SweetAlert.swal({
