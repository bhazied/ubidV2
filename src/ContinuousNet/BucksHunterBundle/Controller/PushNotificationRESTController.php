...
use ContinuousNet\BucksHunterBundle\Entity\PushNotification;
use ContinuousNet\BucksHunterBundle\Form\PushNotificationType;
...
class PushNotificationRESTController extends BaseRESTController
{
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('BucksHunterBundle:PushNotification', 'pn_');
            $qb->leftJoin('ContinuousNet\BucksHunterBundle\Entity\Game', 'game', \Doctrine\ORM\Query\Expr\Join::WITH, 'pn_.game = game.id');
            $qb->leftJoin('ContinuousNet\BucksHunterBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'pn_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BucksHunterBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'pn_.modifierUser = modifier_user.id');
            $textFields = array('pushNotification.title', 'pushNotification.message', 'pushNotification.category', 'pushNotification.sound');
            foreach ($filters as $field => $value) {
                if (substr_count($field, '.') > 1) {
                    if ($value == 'true') {
                        list ($entityName, $listName, $listItem) = explode('.', $field);
                        $qb->andWhere(':'.$listName.'_value MEMBER OF pn_.'.$listName)->setParameter($listName.'_value', $listItem);
                    }
                    continue;
                }
