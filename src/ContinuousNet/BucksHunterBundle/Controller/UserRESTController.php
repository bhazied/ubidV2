...
use ContinuousNet\BucksHunterBundle\Entity\User;
use ContinuousNet\BucksHunterBundle\Form\UserType;
...
class UserRESTController extends BaseRESTController
{
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher)
    {
        try {
            ...
            $qb->from('BucksHunterBundle:User', 'u_');
            $qb->leftJoin('ContinuousNet\BucksHunterBundle\Entity\Country', 'country', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.country = country.id');
            $qb->leftJoin('ContinuousNet\BucksHunterBundle\Entity\Language', 'language', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.language = language.id');
            $qb->leftJoin('ContinuousNet\BucksHunterBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\BucksHunterBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'u_.modifierUser = modifier_user.id');
            $textFields = array('user.username', 'user.phone', 'user.email', 'user.usernameCanonical', 'user.emailCanonical', 'user.name', 'user.nickname', 'user.picture', 'user.address', 'user.zipCode', 'user.city', 'user.facebookKey', 'user.facebookAccessToken', 'user.twitterKey', 'user.twitterAccessToken', 'user.googleKey', 'user.googleAccessToken', 'user.phoneValidationCode', 'user.emailValidationCode', 'user.roles', 'user.confirmationToken', 'user.invitationCode', 'user.usedInvitationCode');
    ...
    public function postAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ROLE_ADMIN') > 0) {
    ...
    private function process(User $entity, $isNew)
    {
        ...
        if (is_null($entity->getRoles()) || empty($entity->getRoles())) {
            $entity->setRoles(array('ROLE_API', 'ROLE_GAMER'));
        }
