...
function($scope, $rootScope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $biddingTypesDataFactory, $usersDataFactory, $translationBiddingTypesDataFactory) {
    ...
    $scope.editorOptions = {
        ...
        allowedContent: true,
        entities: false,
        filebrowserBrowseUrl: '/elfinder/default/translationbiddingtypes',
        filebrowserBrowseRouteParameters: {
            instance: 'default',
            homeFolder: 'translationbiddingtypes',
            editor: 'ckeditor'
        },
        ...
        $timeout(function(){
            ...
            if ($scope.biddingTypes.length == 0) {
                ...
                $biddingTypesDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'order_by[biddingType.name]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.translationBiddingType)) {
                        $scope.translationBiddingType.bidding_type = $scope.translationBiddingType.bidding_type || $scope.biddingTypes[0].id;
                    }
                });
                return def;
            } else {
        ...
        $timeout(function(){
            ...
            if ($scope.users.length == 0) {
                ...
                $usersDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'filters[user.type]': 'Administrator', 'order_by[user.username]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.translationBiddingType)) {
                        $scope.translationBiddingType.creator_user = $scope.translationBiddingType.creator_user || $scope.users[0].id;
                    }
                });
                return def;
            } else {
    ...
    if (angular.isDefined($stateParams.id)) {
        $translationBiddingTypesDataFactory.get({locale: $localStorage.language, id: $stateParams.id}).$promise.then(function(data) {
            $timeout(function(){
