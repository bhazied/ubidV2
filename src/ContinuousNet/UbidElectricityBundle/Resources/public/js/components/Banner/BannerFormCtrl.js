    ...
    $scope.getBannerTypes = function() {
        $timeout(function(){
            ...
            if ($scope.bannerTypes.length == 0) {
                ...
                var def = $q.defer();
                $bannerTypesDataFactory.query({locale: $localeStorage.language, offset: 0, limit: 10000, 'order_by[bannerType.name]': 'asc'}).$promise.then(function(data) {
    ...
    $scope.getBannerPositions = function() {
        $timeout(function(){
            if ($scope.bannerPositions.length == 0) {
                ...
                var def = $q.defer();
                $bannerPositionsDataFactory.query({locale: $localeStorage.language, offset: 0, limit: 10000, 'order_by[bannerPosition.name]': 'asc'}).$promise.then(function(data) {
