...
function($scope, $rootScope, $stateParams, $location, $sce, $timeout, $filter, ngTableParams, $state, $q, $interpolate, $localStorage, toaster, SweetAlert, $tendersDataFactory, $suppliersDataFactory, $usersDataFactory, $bidsDataFactory) {
    ...
    $scope.setCols = function() {
        $scope.cols = [
            ...
            { field: 'tender', 'class': 'has_one', title: $filter('translate')('content.list.fields.TENDER'), sortable: 'tender.title', filter: { 'bid.tender': 'select' }, getValue: $scope.linkValue, filterData: $scope.getTenders(), show: ($scope.getParamValue('tender_id_show_filed', true) && true), displayInList: true, displayField: 'title', state: 'app.marketplace.tendersdetails' },
            ...
            { field: 'reference', title: $filter('translate')('content.list.fields.REFERENCE'), sortable: 'bid.reference', filter: { 'bid.reference': 'text' }, show: ($scope.getParamValue('reference_show_filed', true) && true), displayInList: true, getValue: $scope.textValue },
            ...
            { field: 'status', 'class': 'enum', title: $filter('translate')('content.list.fields.STATUS'), sortable: 'bid.status', filter: { 'bid.status': 'select' }, show: ($scope.getParamValue('status_show_filed', false) && true), displayInList: true, getValue: $scope.interpolatedValue, filterData : $scope.statusesOptions, interpolateExpr: $interpolate('<span class="bidStatus" my-enum="[[ row.status ]]" my-enum-list=\'[[ statuses ]]\'></span>') },
