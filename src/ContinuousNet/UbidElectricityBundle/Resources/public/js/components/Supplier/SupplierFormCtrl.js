...
function($scope, $rootScope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $supplierTypesDataFactory, $countriesDataFactory, $languagesDataFactory, $regionsDataFactory, $usersDataFactory, $categoriesDataFactory, $suppliersDataFactory) {
    ...
    $scope.getSupplierTypes = function() {
        $timeout(function(){
            ...
            if ($scope.supplierTypes.length == 0) {
                ...
                var def = $q.defer();
                $supplierTypesDataFactory.query({locale: $localeStorage.language, offset: 0, limit: 10000, 'order_by[supplierType.name]': 'asc'}).$promise.then(function(data) {
    ...
    $scope.getLanguages = function() {
        $timeout(function(){
            ...
            if ($scope.languages.length == 0) {
                ...
                var def = $q.defer();
                $languagesDataFactory.query({locale: $localeStorage.language, offset: 0, limit: 10000, 'order_by[language.name]': 'asc'}).$promise.then(function(data) {
                    for (var i in data.results) {
                        data.results[i].hidden = false;
                    }
    ...
    if (angular.isDefined($stateParams.id)) {
        $suppliersDataFactory.get({locale: $localeStorage.language, id: $stateParams.id}).$promise.then(function(data) {
            $timeout(function(){
