...
app.controller('PostTypeFormCtrl', ['$scope', '$rootScope', '$state', '$stateParams', '$sce', '$timeout', '$filter', '$uibModal', '$q', '$interpolate', '$localStorage', 'toaster', 'SweetAlert', 'savable', '$usersDataFactory', '$postTypesDataFactory',
function($scope, $rootScope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $usersDataFactory, $postTypesDataFactory) {
    ...
    $scope.editorOptions = {
        ...
        allowedContent: true,
        entities: false,
        filebrowserBrowseUrl: '/elfinder/default/posttypes',
        filebrowserBrowseRouteParameters: {
            instance: 'default',
            homeFolder: 'posttypes',
            editor: 'ckeditor'
        },
        ...
        $timeout(function(){
            ...
            if ($scope.users.length == 0) {
                ...
                $usersDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'filters[user.type]': 'Administrator', 'order_by[user.username]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.postType)) {
                        $scope.postType.creator_user = $scope.postType.creator_user || $scope.users[0].id;
                    }
                });
                return def;
            } else {
    ...
    if (angular.isDefined($stateParams.id)) {
        $postTypesDataFactory.get({locale: $localStorage.language, id: $stateParams.id}).$promise.then(function(data) {
            $timeout(function(){
