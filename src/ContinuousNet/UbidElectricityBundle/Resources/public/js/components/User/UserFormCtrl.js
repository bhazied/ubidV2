...
function($scope, $rootScope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $countriesDataFactory, $languagesDataFactory, $groupsDataFactory, $usersDataFactory) {
    ...
    $scope.roles = [{
        ...
        id: 'ROLE_CALL_CENTER',
        title: $filter('translate')('content.list.fields.rolesoptions.ROLE_CALL_CENTER'),
        css: 'danger'
    }, {
        id: 'ROLE_IT',
    ...
    $scope.getGroups = function() {
        $timeout(function(){
            if ($scope.groups.length == 0) {
                $scope.groups.push({});
                var def = $q.defer();
                $groupsDataFactory.query({locale: $localeStorage.language, offset: 0, limit: 10000, 'order_by[group.name]': 'asc'}).$promise.then(function(data) {
