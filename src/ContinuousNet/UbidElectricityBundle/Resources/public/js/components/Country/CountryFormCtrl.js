...
function($scope, $rootScope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $regionsDataFactory, $usersDataFactory, $countriesDataFactory) {
    ...
    $scope.editorOptions = {
        ...
        allowedContent: true,
        entities: false,
        filebrowserBrowseUrl: '/elfinder/default/countries',
        filebrowserBrowseRouteParameters: {
            instance: 'default',
            homeFolder: 'countries',
            editor: 'ckeditor'
        },
            ...
            if ($scope.regions.length == 0) {
                ...
                $regionsDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'order_by[region.name]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.country)) {
                        $scope.country.region = $scope.country.region || $scope.regions[0].id;
                    }
                });
                return def;
            } else {
        ...
        $timeout(function(){
            ...
            if ($scope.users.length == 0) {
                ...
                $usersDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'filters[user.type]': 'Administrator', 'order_by[user.username]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.country)) {
                        $scope.country.creator_user = $scope.country.creator_user || $scope.users[0].id;
                    }
                });
                return def;
            } else {
