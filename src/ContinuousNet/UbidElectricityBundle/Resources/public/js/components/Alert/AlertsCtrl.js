...
function($scope, $rootScope, $stateParams, $location, $sce, $timeout, $filter, ngTableParams, $state, $q, $interpolate, $localStorage, toaster, SweetAlert, $categoriesDataFactory, $usersDataFactory, $alertsDataFactory) {
    ...
    $scope.setCols = function() {
        $scope.cols = [
            ...
            { field: 'type', 'class': 'enum', title: $filter('translate')('content.list.fields.TYPE'), sortable: 'alert.type', filter: { 'alert.type': 'select' }, show: $scope.getParamValue('type_show_filed', false), getValue: $scope.interpolatedValue, filterData : $scope.typesOptions, interpolateExpr: $interpolate('<span class="alertType" my-enum="[[ row.type ]]" my-enum-list=\'[[ types ]]\'></span>') },
            { field: 'category', 'class': 'has_one', title: $filter('translate')('content.list.fields.CATEGORY'), sortable: 'category.name', filter: { 'alert.category': 'select' }, getValue: $scope.linkValue, filterData: $scope.getCategories(), show: $scope.getParamValue('category_id_show_filed', false), displayField: 'name', state: 'app.lists.categoriesdetails' },
