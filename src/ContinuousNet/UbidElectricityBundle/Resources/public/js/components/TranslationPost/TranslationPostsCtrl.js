...
function($scope, $rootScope, $stateParams, $location, $sce, $timeout, $filter, ngTableParams, $state, $q, $interpolate, $localStorage, toaster, SweetAlert, $postsDataFactory, $usersDataFactory, $translationPostsDataFactory) {
    ...
    $scope.getPosts = function() {
        ...
        if ($scope.posts.length == 0) {
            ...
            var def = $q.defer();
            $postsDataFactory.query({locale: $localeStorage.language, offset: 0, limit: 10000, 'order_by[post.id]': 'desc'}).$promise.then(function(data) {
                $timeout(function(){
    ...
    $scope.setCols = function() {
        $scope.cols = [
            ...
            { field: 'post', 'class': 'has_one', title: $filter('translate')('content.list.fields.POST'), sortable: 'post.title', filter: { 'translationPost.post': 'select' }, getValue: $scope.linkValue, filterData: $scope.getPosts(), show: $scope.getParamValue('post_id_show_filed', true), displayInList: true, displayField: 'title', state: 'app.news.postsdetails' },
            ...
            { field: 'created_at', title: $filter('translate')('content.list.fields.CREATEDAT'), sortable: 'translationPost.createdAt', filter: { 'translationPost.createdAt': 'text' }, show: $scope.getParamValue('created_at_show_filed', false), displayInList: true, getValue: $scope.evaluatedValue, valueFormatter: 'date:\''+$filter('translate')('formats.DATETIME')+'\''},
