...
function($scope, $rootScope, $stateParams, $location, $sce, $timeout, $filter, ngTableParams, $state, $q, $interpolate, $localStorage, toaster, SweetAlert, $postsDataFactory, $usersDataFactory, $translationPostsDataFactory) {
    ...
    $scope.getPosts = function() {
        ...
        if ($scope.posts.length == 0) {
            ...
            var def = $q.defer();
            $postsDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'order_by[post.id]': 'desc'}).$promise.then(function(data) {
                $timeout(function(){
    ...
    $scope.setCols = function() {
        $scope.cols = [
            { field: 'id', title: $filter('translate')('content.list.fields.ID'), sortable: 'translationPost.id', filter: { 'translationPost.id': 'number' }, show: ($scope.getParamValue('id_show_filed', true) && true), displayInList: true, getValue: $scope.textValue },
            { field: 'post', 'class': 'has_one', title: $filter('translate')('content.list.fields.POST'), sortable: 'post.title', filter: { 'translationPost.post': 'select' }, getValue: $scope.linkValue, filterData: $scope.getPosts(), show: ($scope.getParamValue('post_id_show_filed', true) && true), displayInList: true, displayField: 'title', state: 'app.news.postsdetails' },
            { field: 'locale', title: $filter('translate')('content.list.fields.LOCALE'), sortable: 'translationPost.locale', filter: { 'translationPost.locale': 'text' }, show: ($scope.getParamValue('locale_show_filed', true) && true), displayInList: true, getValue: $scope.textValue },
            { field: 'title', title: $filter('translate')('content.list.fields.TITLE'), sortable: 'translationPost.title', filter: { 'translationPost.title': 'text' }, show: ($scope.getParamValue('title_show_filed', true) && true), displayInList: true, getValue: $scope.textValue },
            { field: 'content', title: $filter('translate')('content.list.fields.CONTENT'), sortable: 'translationPost.content', filter: { 'translationPost.content': 'text' }, show: ($scope.getParamValue('content_show_filed', false) && true), displayInList: true, getValue: $scope.textValue },
            { field: 'meta_title', title: $filter('translate')('content.list.fields.METATITLE'), sortable: 'translationPost.metaTitle', filter: { 'translationPost.metaTitle': 'text' }, show: ($scope.getParamValue('meta_title_show_filed', false) && true), displayInList: true, getValue: $scope.textValue },
            { field: 'meta_description', title: $filter('translate')('content.list.fields.METADESCRIPTION'), sortable: 'translationPost.metaDescription', filter: { 'translationPost.metaDescription': 'text' }, show: ($scope.getParamValue('meta_description_show_filed', false) && true), displayInList: true, getValue: $scope.textValue },
            { field: 'meta_keywords', title: $filter('translate')('content.list.fields.METAKEYWORDS'), sortable: 'translationPost.metaKeywords', filter: { 'translationPost.metaKeywords': 'text' }, show: ($scope.getParamValue('meta_keywords_show_filed', false) && true), displayInList: true, getValue: $scope.textValue },
            ...
            { field: 'creator_user', 'class': 'has_one', title: $filter('translate')('content.list.fields.CREATORUSER'), sortable: 'creator_user.username', filter: { 'translationPost.creatorUser': 'select' }, getValue: $scope.linkValue, filterData: $scope.getUsers(), show: ($scope.getParamValue('creator_user_show_filed', false) && true), displayInList: true, displayField: 'username', state: 'app.access.usersdetails' },
            { field: 'created_at', title: $filter('translate')('content.list.fields.CREATEDAT'), sortable: 'translationPost.createdAt', filter: { 'translationPost.createdAt': 'text' }, show: ($scope.getParamValue('created_at_show_filed', false) && true), displayInList: true, getValue: $scope.evaluatedValue, valueFormatter: 'date:\''+$filter('translate')('formats.DATETIME')+'\''},
            { field: 'modifier_user', 'class': 'has_one', title: $filter('translate')('content.list.fields.MODIFIERUSER'), sortable: 'modifier_user.username', filter: { 'translationPost.modifierUser': 'select' }, getValue: $scope.linkValue, filterData: $scope.getUsers(), show: ($scope.getParamValue('modifier_user_show_filed', false) && true), displayInList: true, displayField: 'username', state: 'app.access.usersdetails' },
            { field: 'modified_at', title: $filter('translate')('content.list.fields.MODIFIEDAT'), sortable: 'translationPost.modifiedAt', filter: { 'translationPost.modifiedAt': 'text' }, show: ($scope.getParamValue('modified_at_show_filed', false) && true), displayInList: true, getValue: $scope.evaluatedValue, valueFormatter: 'date:\''+$filter('translate')('formats.DATETIME')+'\''},
