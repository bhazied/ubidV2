...
function($scope, $rootScope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $supplierTypesDataFactory, $usersDataFactory, $translationSupplierTypesDataFactory) {
        ...
        $timeout(function(){
            ...
            if ($scope.supplierTypes.length == 0) {
                ...
                $supplierTypesDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'order_by[supplierType.name]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.translationSupplierType)) {
                        $scope.translationSupplierType.supplier_type = $scope.translationSupplierType.supplier_type || $scope.supplierTypes[0].id;
                    }
                });
                return def;
            } else {
        ...
        $timeout(function(){
            ...
            if ($scope.users.length == 0) {
                ...
                $usersDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'filters[user.type]': 'Administrator', 'order_by[user.username]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.translationSupplierType)) {
                        $scope.translationSupplierType.creator_user = $scope.translationSupplierType.creator_user || $scope.users[0].id;
                    }
                });
                return def;
            } else {
    ...
    if (angular.isDefined($stateParams.id)) {
        $translationSupplierTypesDataFactory.get({locale: $localStorage.language, id: $stateParams.id}).$promise.then(function(data) {
            $timeout(function(){
