...
function($scope, $rootScope, $stateParams, $location, $sce, $timeout, $filter, ngTableParams, $state, $q, $interpolate, $localStorage, toaster, SweetAlert, $menuLinksDataFactory, $usersDataFactory, $translationMenuLinksDataFactory) {
    ...
    $scope.getMenuLinks = function() {
        ...
        if ($scope.menuLinks.length == 0) {
            ...
            var def = $q.defer();
            $menuLinksDataFactory.query({locale: $localeStorage.language, offset: 0, limit: 10000, 'order_by[menuLink.id]': 'desc'}).$promise.then(function(data) {
                $timeout(function(){
    ...
    $scope.setCols = function() {
        $scope.cols = [
            ...
            { field: 'menu_link', 'class': 'has_one', title: $filter('translate')('content.list.fields.MENULINK'), sortable: 'menu_link.name', filter: { 'translationMenuLink.menuLink': 'select' }, getValue: $scope.linkValue, filterData: $scope.getMenuLinks(), show: $scope.getParamValue('menu_link_id_show_filed', true), displayInList: true, displayField: 'name', state: 'app.settings.menulinksdetails' },
            ...
            { field: 'created_at', title: $filter('translate')('content.list.fields.CREATEDAT'), sortable: 'translationMenuLink.createdAt', filter: { 'translationMenuLink.createdAt': 'text' }, show: $scope.getParamValue('created_at_show_filed', false), displayInList: true, getValue: $scope.evaluatedValue, valueFormatter: 'date:\''+$filter('translate')('formats.DATETIME')+'\''},
