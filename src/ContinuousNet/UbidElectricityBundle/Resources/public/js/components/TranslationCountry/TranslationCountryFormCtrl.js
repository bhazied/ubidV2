...
app.controller('TranslationCountryFormCtrl', ['$scope', '$rootScope', '$state', '$stateParams', '$sce', '$timeout', '$filter', '$uibModal', '$q', '$interpolate', '$localStorage', 'toaster', 'SweetAlert', 'savable', '$countriesDataFactory', '$usersDataFactory', '$translationCountriesDataFactory',
function($scope, $rootScope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $countriesDataFactory, $usersDataFactory, $translationCountriesDataFactory) {
    ...
    $scope.editorOptions = {
        ...
        allowedContent: true,
        entities: false,
        filebrowserBrowseUrl: '/elfinder/default/translationcountries',
        filebrowserBrowseRouteParameters: {
            instance: 'default',
            homeFolder: 'translationcountries',
            editor: 'ckeditor'
        },
        ...
        $timeout(function(){
            ...
            if ($scope.countries.length == 0) {
                ...
                $countriesDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'order_by[country.name]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.translationCountry)) {
                        $scope.translationCountry.country = $scope.translationCountry.country || $scope.countries[0].id;
                    }
                });
                return def;
            } else {
        ...
        $timeout(function(){
            ...
            if ($scope.users.length == 0) {
                ...
                $usersDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'filters[user.type]': 'Administrator', 'order_by[user.username]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.translationCountry)) {
                        $scope.translationCountry.creator_user = $scope.translationCountry.creator_user || $scope.users[0].id;
                    }
                });
                return def;
            } else {
