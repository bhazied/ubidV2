 ...
 * @see      SectorRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("Sector")
 */
class SectorRESTController extends BaseRESTController
{
    /**
     * Get a Sector entity
     *
     ...
     *
     */
    public function getAction(Sector $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new Sector());
                ...
                if (!empty($memberOfCondition['items'])) {
                    if ($memberOfCondition['operator'] == 'or') {
                        ...
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $orX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'sector.'.$listName));
                        ...
                        $andX = $qb->expr()->andX();
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $andX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'sector.'.$listName));
                            $qb->setParameter($listName.'_value_'.$i, $item);
                        }
                        $qb->andWhere($andX);
                    }
    ...
    public function postAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new Sector();
        $form = $this->createForm(new SectorType(), $entity, array('method' => $request->getMethod()));
     ...
     * @return Response
     */
    public function putAction(Request $request, Sector $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Sector entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, Sector $entity)
    {
     ...
     * @return Response
     */
    public function deleteAction(Request $request, Sector $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
            $em->remove($entity);
