 ...
 * @see      BiddingTypeRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("BiddingType")
 */
class BiddingTypeRESTController extends BaseRESTController
{
    /**
     * Get a Bidding Type entity
     *
     ...
     *
     */
    public function getAction(BiddingType $entity)
    {
    ...
    {
        try {
            $this->createSubDirectory(new BiddingType());
                ...
                if (!empty($memberOfCondition['items'])) {
                    if ($memberOfCondition['operator'] == 'or') {
                        ...
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $orX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'biddingType.'.$listName));
                        ...
                        $andX = $qb->expr()->andX();
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $andX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'biddingType.'.$listName));
                            $qb->setParameter($listName.'_value_'.$i, $item);
                        }
                        $qb->andWhere($andX);
                    }
     ...
     * @return Response
     */
    public function putAction(Request $request, BiddingType $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Bidding Type entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, BiddingType $entity)
    {
        return $this->putAction($request, $entity);
    }

    /**
     * Delete a Bidding Type entity.
     *
     * @View(statusCode=204)
     *
     * @param Request $request
