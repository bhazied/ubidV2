...
/**
 * Translation Tender Type REST Controller
 * 
 ...
 * @see      TranslationTenderTypeRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("TranslationTenderType")
 */
class TranslationTenderTypeRESTController extends BaseRESTController
{
    /**
     * Get a Translation Tender Type entity
     *
     ...
     *
     */
    public function getAction(TranslationTenderType $entity)
    {
    ...
    /**
     * Get all Translation Tender Type entities.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
    ...
    {
        try {
            $this->createSubDirectory(new TranslationTenderType());
                ...
                if (!empty($memberOfCondition['items'])) {
                    if ($memberOfCondition['operator'] == 'or') {
                        ...
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $orX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'translationTenderType.'.$listName));
                        ...
                        $andX = $qb->expr()->andX();
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $andX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'translationTenderType.'.$listName));
                            $qb->setParameter($listName.'_value_'.$i, $item);
                        }
                        $qb->andWhere($andX);
                    }
    ...
    /**
     * Create a Translation Tender Type entity.
     *
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new TranslationTenderType();
        $form = $this->createForm(new TranslationTenderTypeType(), $entity, array('method' => $request->getMethod()));
    ...
    /**
     * Update a Translation Tender Type entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function putAction(Request $request, TranslationTenderType $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Translation Tender Type entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, TranslationTenderType $entity)
    {
        return $this->putAction($request, $entity);
    }

    /**
     * Delete a Translation Tender Type entity.
     *
     * @View(statusCode=204)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function deleteAction(Request $request, TranslationTenderType $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
            $em->remove($entity);
