...
/**
 * Translation Sector REST Controller
 * 
 ...
 * @see      TranslationSectorRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("TranslationSector")
 */
class TranslationSectorRESTController extends BaseRESTController
{
    /**
     * Get a Translation Sector entity
     *
     ...
     *
     */
    public function getAction(TranslationSector $entity)
    {
    ...
    /**
     * Get all Translation Sector entities.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
    ...
    {
        try {
            $this->createSubDirectory(new TranslationSector());
                ...
                if (!empty($memberOfCondition['items'])) {
                    if ($memberOfCondition['operator'] == 'or') {
                        ...
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $orX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'translationSector.'.$listName));
                        ...
                        $andX = $qb->expr()->andX();
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $andX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'translationSector.'.$listName));
                            $qb->setParameter($listName.'_value_'.$i, $item);
                        }
                        $qb->andWhere($andX);
                    }
    ...
    /**
     * Create a Translation Sector entity.
     *
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $entity = new TranslationSector();
        $form = $this->createForm(new TranslationSectorType(), $entity, array('method' => $request->getMethod()));
    ...
    /**
     * Update a Translation Sector entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function putAction(Request $request, TranslationSector $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Translation Sector entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, TranslationSector $entity)
    {
        return $this->putAction($request, $entity);
    }

    /**
     * Delete a Translation Sector entity.
     *
     * @View(statusCode=204)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function deleteAction(Request $request, TranslationSector $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
            $em->remove($entity);
