...
/**
 * Supplier Product REST Controller
 * 
 ...
 * @see      SupplierProductRESTController
 * @since      Class available since Release 1.0
 * @access    public
 * @RouteResource("SupplierProduct")
 */
class SupplierProductRESTController extends BaseRESTController
{
    /**
     * Get a Supplier Product entity
     *
     ...
     *
     */
    public function getAction(SupplierProduct $entity)
    {
    ...
    /**
     * Get all Supplier Product entities.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
    ...
    {
        try {
            $this->createSubDirectory(new SupplierProduct());
                ...
                if (!empty($memberOfCondition['items'])) {
                    if ($memberOfCondition['operator'] == 'or') {
                        ...
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $orX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'supplierProduct.'.$listName));
                        ...
                        $andX = $qb->expr()->andX();
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $andX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'supplierProduct.'.$listName));
                            $qb->setParameter($listName.'_value_'.$i, $item);
                        }
                        $qb->andWhere($andX);
                    }
    ...
    /**
     * Create a Supplier Product entity.
     *
    ...
    public function postAction(Request $request)
    {
        ...
        $form = $this->createForm(new SupplierProductType(), $entity, array('method' => $request->getMethod()));
    ...
    /**
     * Update a Supplier Product entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function putAction(Request $request, SupplierProduct $entity)
    {
        try {
            $em = $this->getDoctrine()->getManager();
    ...
    /**
     * Partial Update to a Supplier Product entity.
     *
     * @View(serializerEnableMaxDepthChecks=true)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function patchAction(Request $request, SupplierProduct $entity)
    {
        return $this->putAction($request, $entity);
    }

    /**
     * Delete a Supplier Product entity.
     *
     * @View(statusCode=204)
     *
     * @param Request $request
     ...
     * @return Response
     */
    public function deleteAction(Request $request, SupplierProduct $entity)
    {
        try {
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
