...
class TranslationCategoryRESTController extends BaseRESTController
{
    ...
    {
        try {
            $this->createSubDirectory(new TranslationCategory());
            ...
            $qb->from('UbidElectricityBundle:TranslationCategory', 'translationCategory');
            $qb->leftJoin('ContinuousNet\UbidElectricityBundle\Entity\Category', 'category', \Doctrine\ORM\Query\Expr\Join::WITH, 'translationCategory.category = category.id');
            $qb->leftJoin('ContinuousNet\UbidElectricityBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'translationCategory.creatorUser = creator_user.id');
            $qb->leftJoin('ContinuousNet\UbidElectricityBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'translationCategory.modifierUser = modifier_user.id');
            $textFields = array('translationCategory.locale', 'translationCategory.name', 'translationCategory.description', 'translationCategory.metaTitle', 'translationCategory.metaDescription', 'translationCategory.metaKeywords', 'translationCategory.tendersDescription', 'translationCategory.tendersMetaTitle', 'translationCategory.tendersMetaDescription', 'translationCategory.tendersMetaKeywords', 'translationCategory.consultationsDescription', 'translationCategory.consultationsMetaTitle', 'translationCategory.consultationsMetaDescription', 'translationCategory.consultationsMetaKeywords', 'translationCategory.buyersDescription', 'translationCategory.buyersMetaTitle', 'translationCategory.buyersMetaDescription', 'translationCategory.buyersMetaKeywords', 'translationCategory.suppliersDescription', 'translationCategory.suppliersMetaTitle', 'translationCategory.suppliersMetaDescription', 'translationCategory.suppliersMetaKeywords');
                ...
                if (!empty($memberOfCondition['items'])) {
                    if ($memberOfCondition['operator'] == 'or') {
                        ...
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $orX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'translationCategory.'.$listName));
                        ...
                        $andX = $qb->expr()->andX();
                        foreach ($memberOfCondition['items'] as $i => $item) {
                            $andX->add($qb->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'translationCategory.'.$listName));
                            $qb->setParameter($listName.'_value_'.$i, $item);
                        }
                        $qb->andWhere($andX);
                    }
            ...
            $qbList = clone $qb;
            $qb->select('count(translationCategory.id)');
            $data['inlineCount'] = $qb->getQuery()->getSingleScalarResult();
            foreach ($order_by as $field => $direction) {
                $qbList->addOrderBy($field, $direction);
            }
            $qbList->select('translationCategory');
            $qbList->setMaxResults($limit);
            $qbList->setFirstResult($offset);
            $qbList->groupBy('translationCategory.id');
