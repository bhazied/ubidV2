...
app.controller('SessionFormCtrl', ['$scope', '$rootScope', '$state', '$stateParams', '$sce', '$timeout', '$filter', '$uibModal', '$q', '$interpolate', '$localStorage', 'toaster', 'SweetAlert', 'savable', '$usersDataFactory', '$sessionsDataFactory',
function($scope, $rootScope, $state, $stateParams, $sce, $timeout, $filter, $uibModal, $q, $interpolate, $localStorage, toaster, SweetAlert, savable, $usersDataFactory, $sessionsDataFactory) {
    ...
    $scope.editorOptions = {
        ...
        allowedContent: true,
        entities: false,
        filebrowserBrowseUrl: '/elfinder/default/sessions',
            ...
            if ($scope.users.length == 0) {
                ...
                $usersDataFactory.query({locale: $localStorage.language, offset: 0, limit: 10000, 'filters[user.type]': 'Administrator', 'order_by[user.username]': 'asc'}).$promise.then(function(data) {
                    ...
                    if (angular.isDefined($scope.session)) {
                        $scope.session.creator_user = $scope.session.creator_user || $scope.users[0].id;
                    }
                });
                return def;
            } else {
